...
def check_edge_worms(image_seg, image_skl, kernel_size):
    ...
    # edge_final = cv2.dilate(edge_final.astype('uint8'), kernel, iterations=2)
    # edge_final = cv2.erode(edge_final.astype('uint8'), kernel, iterations=1)

    # Fill holes
    thresh = ((((image_seg[:, :, 0] > 0) * 1) * ((edge_final == 0) * 1)) * 255).astype('uint8')
    ...
    ret, sure_fg = cv2.threshold(dist_transform, 0.32 * dist_transform.max(), 255, 0)  # up (low), down(more)
    ...
    labels_F = watershed(-dist_transform, markers, mask=thresh)
    labels_skl = measure.label(image_skl, background=0)

    edge_watershed = np.zeros((h, w))
    for i in range(labels_skl.max() + 1):
        if i != 0:
            skl_i = (labels_skl == i) * 1
            list_parts = labels_list(skl_i * labels_F)

            new_part = np.zeros((h, w))
            for j in list_parts:
                new_part = new_part + ((labels_F == j) * 1)
            edge_i = build_edge(new_part)
            edge_watershed = edge_watershed + ((edge_i > 0) * 1)

    # for i in range(labels_F.max()):
    #     if i != 0:
    #         edge_i = build_edge((labels_F == i) * 1)
    #         edge_watershed = edge_watershed + edge_i
    # plt.imshow(edge_watershed)
    # plt.show()
    #
    edge_final_final = (((edge_final + edge_watershed) > 0) * 255).astype('uint8')
    # overlap_new = cv2.subtract(edge_final_final, edge_final)

    # plt.figure(figsize=(15, 15))
    # plt.subplot(131)
    # plt.imshow(edge_final)
    # plt.subplot(132)
    # plt.imshow(edge_final_final)
    # plt.subplot(133)
    # plt.imshow(overlap_new)

    return edge_final_final
